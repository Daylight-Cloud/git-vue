...
// 使用 ref 定义 stats 和 borrowTrend
const stats = ref({ books: 0, borrowed: 0, users: 0, inLibrary: 0 });
...
const renderLine = () => {
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: borrowTrend.value.map(i => formatTrendLabel(i.date)),
      axisLine: { lineStyle: { color: '#e0e0e0' } },
      axisLabel: { color: '#666', fontSize: 12 }
    },
    yAxis: {
...
const fetchStats = async () => {
  try {
    const response = await service.get('/stats');
...
const fetchTrend = async () => {
  try {
    const response = await service.get('/borrow-trend');
    borrowTrend.value = Array.isArray(response) ? response : [];
  } catch (error) {
...
// 监听 stats / borrowTrend 变化并重新渲染图表
watch(stats, () => {
  renderPie();
  renderBar();
});
